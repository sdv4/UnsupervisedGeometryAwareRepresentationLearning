...
class IgniteTestNVS(train_encodeDecode.IgniteTrainNVS):
    def run(self, config_dict):
        ...
        def rotation_matrix_xzy(theta, phi, psi):
            Ax = np.matrix([[1, 0, 0],
        ...
        def update_rotation(event):
            ...
            input_dict['external_rotation_global'] = torch.from_numpy(rotation_matrix_xzy(theta=0, phi=0, psi=rot)).float().to(device)
            input_dict['external_rotation_cam'] = torch.from_numpy(np.eye(3)).float().to(device) # torch.from_numpy(rotation_matrix_xzy(theta=0, phi=rot, psi=0)).float().cuda()
            predict()
            update_figure()
        ...
        def next_button_pressed(event):
            next_image()
            predict()
            update_figure()
        button_next.on_clicked(next_button_pressed)
