...
class EncoderDecoder(GeometryAwareLearner):
    """
        ...
        Args:
            ...
            is_deconv (boolean): if True, increase dimensions in decoder via deconv, else via bilinear interpolation
            upper_bilinear (boolean): decoder will increase dimensionality via bilinear interpolation TODO: say where
            lower_bilinear (boolean): encoder will reduce dimensionality via bilinear interpolation TODO: say where
            in_channels (int): number of input channels to the decoder. 3 for RBG images
            is_batchnorm (boolean): add batch-normalization operation after activation function in the encoder
            skip_background (boolean): #TODO
    ...
    def __init__(self,
        ...
        setattr(self, 'final_stage' + str(ns), nn.Conv2d(self.filters[0], out_channels, 1))
        ...
        if not rotation_by_user:
            if self.shuffle_fg and self.training:
                for i in range(0, num_pose_subbatches):
                    super()._shuffle_segment(shuffled_appearance,
                                     i*self.subbatch_size,
                                     (i+1)*self.subbatch_size,
                                     self.training,
                                     self.num_cameras)
                for i in range(0, num_pose_subbatches//2): # flip first with second subbatch
                    super()._flip_segment(shuffled_appearance, i*2*self.subbatch_size, self.subbatch_size)
            if self.shuffle_3d:
                for i in range(0, num_pose_subbatches):
                    super()._shuffle_segment(shuffled_pose,
                                     i*self.subbatch_size,
                                     (i+1)*self.subbatch_size,
                                     self.training,
                                     self.num_cameras)
