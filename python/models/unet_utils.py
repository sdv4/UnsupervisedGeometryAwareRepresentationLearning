...
class unetUpNoSKipXXXXXXXX(nn.Module):
    def __init__(self, in_size, out_size, is_deconv, padding):
    ...
    def forward(self, inputs2):
        ...
        outputs2 = self.up(inputs2)
        #outputs2X = self.upX(inputs2)
        #print('outputs2X.size()',outputs2.size(),outputs2X.size())
        return self.conv(outputs2)

class LiftNetUp(nn.Module):
    def __init__(self, in_size, out_size, is_deconv, filter_size, padding):
        super(LiftNetUp, self).__init__()
        self.conv = nn.Sequential(nn.Conv2d(in_size*2, out_size, filter_size, 1, padding), nn.ReLU(),)
        if is_deconv:
            self.up = nn.ConvTranspose2d(in_size, in_size, kernel_size=2, stride=2)
        else:
            self.up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True),
